...
@Composable
fun TransactionTagFilterSettingsScreen(navController: NavController) {
    val vm: TransactionTagFilterSettingsViewModel = hiltViewModel()
    val state by vm.uiState.collectAsState()

    Scaffold(topBar = {
        TopAppBar(title = { Text("Transaction Tag Filter") }, navigationIcon = {
        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text("Select tags to filter transactions", style = MaterialTheme.typography.bodyMedium)
            ...
            LazyColumn(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                items(state.availableTags) { tag ->
                    ListItem(
                        ...
                        trailingContent = { Checkbox(checked = tag.isSelected, onCheckedChange = { vm.toggleTag(tag.id) }) }
                    )
                }
            }
        }
