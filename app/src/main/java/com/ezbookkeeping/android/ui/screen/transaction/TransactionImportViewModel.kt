...
class TransactionImportViewModel @Inject constructor() : ViewModel() {
    ...
    fun goToStep(step: ImportStep) { _uiState.update { it.copy(step = step) } }
    fun setColumnMapping(col: String, value: String) { _uiState.update { it.copy(columnMapping = it.columnMapping + (col to value)) } }
    ...
    fun pickFile() {
        ...
        ), step = ImportStep.DEFINE_COLUMNS) }
    }

    fun confirmImport() {
        _uiState.update { it.copy(isImporting = true) }
        _uiState.update { it.copy(isImporting = false, importResult = "Successfully imported ${_uiState.value.previewData.size} transactions", step = ImportStep.EXECUTE) }
    }
}
