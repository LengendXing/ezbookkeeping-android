...
fun CategoryEditScreen(navController: NavController, categoryId: Int? = null) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(if (state.isEdit) "Edit Category" else "New Category") }, navigationIcon = { TextButton(onClick = { navController.popBackStack() }) { Text("Cancel") } }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            OutlinedTextField(value = state.name, onValueChange = vm::onNameChange, label = { Text("Category Name") }, modifier = Modifier.fillMaxWidth(), singleLine = true)
